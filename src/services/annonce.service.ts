...
import {RootObject, Update2} from "../models/update2";
...
export class AnnonceService {
  ...
  public  Delete  (OrdreID, BladID): Observable<HttpResponse<any>> {
    let headers = new HttpHeaders({
      'Content-Type': 'application/json'});
    const url = this.baseurl + "annoncekontrol/" + OrdreID + "/" + BladID;
    return  this.http.delete<HttpResponse<any>>(url, {headers: headers}).pipe();
  }

  public  InsertSQL(insert: WebInsert): Observable<any> {
    let headers = new HttpHeaders({
      'Content-Type': 'application/json'});
    const url = this.baseurl +"InsertSQL/";
    return  this.http.post<any>(url, JSON.stringify(insert),{headers: headers}).pipe()
  }

  public  UpdateAnnonceKontrol(update2: RootObject): Observable<number> {
    let headers = new HttpHeaders({
      'Content-Type': 'application/json'});
    const url = this.baseurl + "UpdateAnnonceKontrol/";
    return  this.http.put<number>(url, JSON.stringify(update2),{headers: headers}).pipe()
  }
  ...
  public UpdateMediePlan(update: Update): Observable<number> {
    ...
    return  this.http.put<number>(url, JSON.stringify(update),{headers: headers}).pipe();
  }
...
return this.http.put<number>(url, JSON.stringify(update3), {headers: headers}).pipe();
  }
